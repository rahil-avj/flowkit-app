...
- **`"files"` is an allowlist**, not a denylist — only these paths (and their contents) end up in what npm packs, whether via `npm publish`, a `file:` dependency, or a git dependency. Anything not listed (`eslint.config.js`, `.prettierrc`, `vitest.config.ts`, etc.) is excluded _by omission_ — no negation needed.
