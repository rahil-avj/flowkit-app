...
export default function <PascalName>Screen({ onNext, onBack, db }: PageProps) {
  return (
...
`create:flow` creates the directory AND adds the flow to `flowkit.config.ts` flows array + empty `pageOrder` key. `remove:flow` requires `--force` if screens exist.
...
| `add:step`        | `flowkit add:step --flowplan:<id> --screen:<pageId> [--on:<element-id>] [--action:"note"] [--position:<n>] [--workspace:<ws>]` |
...
`add:step` validates that `--screen` references a pageId that exists in the workspace before writing. This is the first layer of flowlint-style checking.
...
| Command                | Syntax                                                                                          |
| ---------------------- | ----------------------------------------------------------------------------------------------- |
| `config:add-screen`    | `flowkit config:add-screen --flow:<flow> --screen:<pageId> [--position:<n>] [--workspace:<ws>]` |
| `config:remove-screen` | `flowkit config:remove-screen --flow:<flow> --screen:<pageId> [--workspace:<ws>]`               |
...
# → Should error: pageId 'nonexistent' not found in workspace
