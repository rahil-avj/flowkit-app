...
| Always use the right import for the mode you're in   | Repo mode: `@platform/`/`@workspace/` aliases. Consumer mode: import directly from `'flowkit'` — there are no `@platform`/`@workspace` aliases outside this repo. Relative `../../` paths break across the workspace boundary in either mode. |

---

## Workspace format

Workspaces use the **flat flowplan format**: `flows/<flow>/<screen>/<Screen>.tsx` + `flowplans/*.ts`. There is no `_playFlow.ts`, no `router.tsx`, no `projects/` directory (unless you've deliberately opted into the nested-layout `projects` field in `flowkit.config.ts` — see FLOWKIT.md).
...
> **To remove a screen:** `flowkit remove:screen --flow:<flow-id> --name:<screen-slug>` (unregisters it and deletes the directory — safer than a manual `rm -rf`, since it also updates `flowkit.config.ts`).
...
- There is no separate staleness-check command as of this writing — `agent:check` doesn't exist,
  and `flowkit export`'s pre-flight checks (TypeScript, ESLint, FlowPlan validation) don't cover
  agent-file staleness either. Re-run `agent:sync` yourself after any platform change.
