...
- **Bare vs. composite screen ids — don't conflate them.** Since the `flows/`→`flowBook/` / `flowplans/`→`flowStories/` folder rename (see `src/shared/utils/screenPathIdentity.js`), the registered screen id is a **composite** `${flowId}-${screenId}` (collision-proof across flows) — flowplan step `screenId` values and any other cross-flow reference use this composite form. `workspace.ts`'s `screenOrder` map is the one exception: it stays **bare**, since that map is already flow-scoped by its own outer key (`screenOrder['onboarding-flow'] = ['welcome-screen', ...]`). Writing a bare id into a flowplan step, or a composite id into `screenOrder`, won't throw — it just silently fails to resolve.
